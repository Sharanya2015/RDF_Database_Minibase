...
public class IoBuf implements GlobalConst{
  ...
  public void reread()
	   ...
	   Exception
    {
      ...
      mode = READ_BUFFER;
      if (flushed)                   // Has the output buffer been flushed?
	{
	  // flush all the remaining Tuple to disk.
	  flush();
	  i_buf.init(_temp_fd, _bufs, _n_pages, t_size, (int)t_written);
	}
      else
	{
	  // All the Tuple are in the buffer, just read them out.
	  t_rd_from_pg = 0;
	  curr_page    = 0;
	}
    }   
  ...
  private  int  t_per_pg,              // # of Tuple that fit in 1 page
    t_in_buf;                        // # of Tuple that fit in the buffer
  private  int  t_wr_to_pg,          // # of Tuple written to current page
    t_wr_to_buf;                      // # of Tuple written to buffer.
  ...
  private  long t_written;           // # of Tuple written so far
  ...
  private  int  mode;
  private  int  t_rd_from_pg;      // # of Tuple read from current page
