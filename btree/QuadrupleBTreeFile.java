/*
 * @(#) QuadrupleBTreeFile.java 
 * This class is derieved from BTreeFile.java  98/03/24
 ...
 * This is the main definition of class QuadrupleBTreeFile, which derives from
...
public class QuadrupleBTreeFile extends QuadrupleIndexFile
	 ...
	 * Quadruple b-tree during its operations.
	 * 
	 ...
	 * 
	 * @return Return a QuadrupleBTreeHeaderPage object that is the header page
	 *         of this Quadruplebtree file.
	 */
	 ...
	 * 
	 * @param filename the Quadruple B+ tree file name. Input parameter.
	 * @exception GetFileEntryException  can not ger the file from DB
	 * @exception PinPageException       failed when pin a page
	 * @exception ConstructPageException Quadruple BT page constructor failed
	 */
	public QuadrupleBTreeFile(String filename)
		...
		/*
		 *
		 * - headerPageId is the PageId of this QuadrupleBTreeFile's header page;
	...
	/**
	 * Destroy entire Quadruple B+ tree file.
	 * 
	 * @exception IOException              error from the lower layer
	 ...
	 * @exception ConstructPageException   error in Quadruple BT page constructor
	 * @exception PinPageException         failed when pin a page
	 */
	...
	private void _destroyFile(PageId pageno)
		...
		if (sortedPage.getType() == NodeType.INDEX) {
			...
			PageId childId;
			KeyDataEntry entry;
			for (entry = indexPage.getFirst(qid); entry != null; entry = indexPage.getNext(qid)) {
	...
	/**
	 * insert quadruple with the given key and qid
	 * 
	 * @param key the key of the quadruple. Input parameter.
	 * @param qid the qid of the quadruple. Input parameter.
	 ...
	 * @exception ConstructPageException  error in QuadrupleBT page constructor
	 * @exception UnpinPageException      error when unpin a page
...

	{
		...
		if (headerPage.get_rootId().pid == INVALID_PAGE) {
			...
			newRootPage.insertQuadruple(key, qid);
...

	{
		...
		// - pageType == LEAF:
		// try to insert pair (key, qid), maybe split
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			if (currentIndexPage.available_space() >= QuadrupleBT.getKeyDataLength(upEntry.key, NodeType.INDEX)) {
			...
			for (tmpEntry = currentIndexPage.getFirst(delQid); tmpEntry != null; tmpEntry = currentIndexPage
					.getFirst(delQid)) {
				newIndexPage.insertKey(tmpEntry.key,
						((IndexData) tmpEntry.data).getData());
				currentIndexPage.deleteSortedQuadruple(delQid);
			}

			// ASSERTIONS:
			// - currentIndexPage empty
			// - newIndexPage holds all former quadruples from currentIndexPage

			// we will try to make an equal split
			QID firstQid = new QID();
			KeyDataEntry undoEntry = null;
			for (tmpEntry = newIndexPage.getFirst(firstQid); (currentIndexPage.available_space() > newIndexPage
					.available_space()); tmpEntry = newIndexPage.getFirst(firstQid)) {
				// now insert the <key,pageId> pair on the new
				// index page
				undoEntry = tmpEntry;
				currentIndexPage.insertKey(tmpEntry.key,
						((IndexData) tmpEntry.data).getData());
				newIndexPage.deleteSortedQuadruple(firstQid);
			}

			// undo the final quadruple
			if (currentIndexPage.available_space() < newIndexPage.available_space()) {
				...
				currentIndexPage.deleteSortedQuadruple(new QID(currentIndexPage.getCurPage(),
			...
			if (QuadrupleBT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
				...
				tmpEntry = QuadrupleBT.getEntryFromBytes(currentIndexPage.getpage(),
				...
				currentIndexPage.deleteSortedQuadruple(new QID(currentIndexPage.getCurPage(), i));
			...
			// delete first quadruple on new index page since it is given up
			newIndexPage.deleteSortedQuadruple(delQid);
			...
			QuadrupleBTLeafPage currentLeafPage = new QuadrupleBTLeafPage(page, headerPage.get_keyType());
			...
			// check whether there can still be entries inserted on that page
			if (currentLeafPage.available_space() >= QuadrupleBT.getKeyDataLength(key, NodeType.LEAF)) {
				// no split has occurred

				currentLeafPage.insertQuadruple(key, qid);
			...
			for (tmpEntry = currentLeafPage.getFirst(firstQid); tmpEntry != null; tmpEntry = currentLeafPage
					.getFirst(firstQid)) {

				newLeafPage.insertQuadruple(tmpEntry.key,
						((QuadrupleLeafData) (tmpEntry.data)).getData());
				currentLeafPage.deleteSortedQuadruple(firstQid);

			}

			// ASSERTIONS:
			// - currentLeafPage empty
			// - newLeafPage holds all former quadruples from currentLeafPage

			KeyDataEntry undoEntry = null;
			for (tmpEntry = newLeafPage.getFirst(firstQid); newLeafPage.available_space() < currentLeafPage
					.available_space(); tmpEntry = newLeafPage.getFirst(firstQid)) {
				undoEntry = tmpEntry;
				currentLeafPage.insertQuadruple(tmpEntry.key,
						((QuadrupleLeafData) tmpEntry.data).getData());
				newLeafPage.deleteSortedQuadruple(firstQid);
			}

			if (QuadrupleBT.keyCompare(key, undoEntry.key) < 0) {
				// undo the final quadruple
				if (currentLeafPage.available_space() < newLeafPage.available_space()) {
					newLeafPage.insertQuadruple(undoEntry.key,
							((QuadrupleLeafData) undoEntry.data).getData());

					currentLeafPage.deleteSortedQuadruple(new QID(currentLeafPage.getCurPage(),
							(int) currentLeafPage.getSlotCnt() - 1));
				}
			}

			// check whether <key, qid>
			// will be inserted
			// on the newly allocated or on the old leaf page

			if (QuadrupleBT.keyCompare(key, undoEntry.key) >= 0) {
				// the new data entry belongs on the new Leaf page
				newLeafPage.insertQuadruple(key, qid);
	 ...
	 * `qid' is IN the data entry; it is not the id of the data entry)
	 * 
	 * @param key the key in pair <key, qid>. Input Parameter.
	 * @param qid the qid in pair <key, qid>. Input Parameter.
	 * @return true if deleted. false if no such quadruple.
	 ...
	 * @exception RecordNotFoundException   error delete a quadruple in a BT page
	 * @exception PinPageException          error when pin a page
	 ...
	 * lo_key is null.
	 * 
	 * Starting quadruple returned in *pstartlid, on page *pppage, which is pinned.
	 *
	 ...
	 * 
	 * @param startlid it will reurn the first qid =< lo_key
	 * 
	...
	QuadrupleBTLeafPage findRunStart(KeyClass lo_key,
		...
		if (pageno.pid == INVALID_PAGE) { // no pages in the BTREE
			...
			// startlid =INVALID_PAGEID ; // the caller
			return pageLeaf;
		}
		...
		while (curEntry == null) {
			...
			if (nextpageno.pid == INVALID_PAGE) {
				// oops, no more quadruples, so set this scan to indicate this.
				return null;
			}
		...
		// ASSERTIONS:
		// - curkey, curLid: contain the first quadruple on the
	...
	private boolean NaiveDelete(KeyClass key, QID qid)
		...
		leafPage = findRunStart(key, curQid); // find first page,qid of key
		if (leafPage == null)
			return false;
		...
		while (true) {
			...
			if (leafPage.delEntry(new KeyDataEntry(key, qid)) == true) {
				...
				// successfully found <key, qid> on this page and deleted it.
		 ...
		 * We should have got true back from delUserlid above. Apparently
		 * the specified <key,qid> data entry does not exist.
		 */
	...
	/*
	 * Status QuadrupleBTreeFile::FullDelete (const void *key, const qid qid)
	 * 
	 * Remove specified data entry (<key, qid>) from an index.
	 *
	 ...
	 * to find the one containing <key,qid>, which we then delete via
	 * QuadrupleBTLeafPage::delUserlid.
	 * 
	 * @return false if no such quadruple; true if succees
	 */
	...
	private KeyClass _Delete(KeyClass key,
		...
		if (sortPage.getType() == NodeType.LEAF) {
			...
			QID delQid;
			// for all Quadruples with key equal to 'key', delete it if its qid = 'qid'
			while ((tmpEntry != null) && (QuadrupleBT.keyCompare(key, tmpEntry.key) >= 0)) {
				// WriteUpdateLog is done in the Quadruplebtleafpage level - to log the
				// deletion of the qid.
				...
				if (leafPage.delEntry(new KeyDataEntry(key, qid))) {
					...
					if ((4 + leafPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
						...
						if (leafPage.numberOfQuadruple() != 0) {
						...
						if (siblingPage.redistribute(leafPage, parentPage,
							...
							return null;
								...
								+ 8 /* 2*sizeof(slot) */ ) >= ((MAX_SPACE - THFPage.DPFIXED)
							...
							for (tmpEntry = rightChild.getFirst(firstQid); tmpEntry != null; tmpEntry = rightChild
									.getFirst(firstQid)) {
								leftChild.insertQuadruple(tmpEntry);
								rightChild.deleteSortedQuadruple(firstQid);
							}
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
				...
				if (indexPage.numberOfQuadruple() == 0) {
			...
			if ((4 /* sizeof slot */ + indexPage.available_space()) <= ((MAX_SPACE - THFPage.DPFIXED) / 2)) {
				...
				int pushKeySize = 0;
				if (direction == 1) {
					pushKeySize = QuadrupleBT.getKeyLength(parentPage.findKey(siblingPage.getFirst(new QID()).key));
				} else if (direction == -1) {
					pushKeySize = QuadrupleBT.getKeyLength(parentPage.findKey(indexPage.getFirst(new QID()).key));
				}
				...
				if (siblingPage.redistribute(indexPage, parentPage,
					...
					// move all entries from rightChild to leftChild
					for (KeyDataEntry tmpEntry = rightChild.getFirst(firstQid); tmpEntry != null; tmpEntry = rightChild
							.getFirst(firstQid)) {
	...
	void trace_children(PageId id)
		...
		if (trace != null) {
			...
			if (sortedPage.getType() == NodeType.INDEX) {
				...
				for (entry = indexPage.getFirst(metaQid); entry != null; entry = indexPage.getNext(metaQid)) {
				...
				for (entry = leafPage.getFirst(metaQid); entry != null; entry = leafPage.getNext(metaQid)) {
