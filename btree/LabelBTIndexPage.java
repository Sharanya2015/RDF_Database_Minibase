 ...
 * 
 * This is derived from BTIndexPage.java
...
public class LabelBTIndexPage extends LabelBTSortedPage {
   ...
   * @exception ConstructPageException error when LabelBTIndexpage constructor
   */
   ...
   *                labelBTIndexPage will correspond to.
   * @param keyType either AttrType.attrInteger or AttrType.attrString.
   *                Input parameter.
   * @exception IOException            error from the lower layer
   * @exception ConstructPageException error when LabelBTIndexpage constructor
  ...
  /*
   * new a page, associate the labelBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   * 
   * @param keyType either AttrType.attrInteger or AttrType.attrString.
   * Input parameter.
   ...
   * @return It returns the LID where the label is inserted;
   *         null if no space left.
   * @exception IndexInsertRecException error when inserting label
   */
  public LID insertKey(KeyClass key, PageId pageNo)
    ...
    try {
      ...
      lid = super.insertLabel(entry);
      return lid;
    } catch (Exception e) {
      throw new IndexInsertRecException(e, "Inserting label failed");
   ...
   * 
   * @exception IndexFullDeleteException if no label deleted or failed by
   * any reason
   * 
   * @return LID of the label deleted. Can not return null.
   */
  LID deleteKey(KeyClass key)
    ...
    try {
      ...
      if (entry == null)
        // it is supposed there is at least a label
        throw new IndexFullDeleteException(null, "No labels found");
   ...
   * LabelBTIndexPage by B++ search algorithm
   * 
   ...
   * which provide an iterator interface to the labels on a LabelBTIndexPage.
   * 
   * @param lid It will be modified and the first LID in the index page
   ...
   * iterator interface to the labels on a LabelBTIndexPage.
   * 
   * @param lid It will be modified and next LID will be passed out by itself.
  ...
  boolean adjustKey(KeyClass newKey, KeyClass oldKey)
    ...
    try {
      ...
      if (lid == null)
        throw new IndexFullDeleteException(null, "Lid is null");
    ...
    try {
      ...
      if (direction == -1) { // 'this' is the left sibling of indexPage
        if ((getSlotLength(getSlotCnt() - 1) + available_space()) > ((MAX_SPACE - DPFIXED) / 2)) {
          ...
          // get its sibling's first label's key
          ...
          if (deleteSortedLabel(delLid) == false)
            throw new RedistributeException(null, "Delete label failed");
