...
public class LabelBTreeFile extends LabelIndexFile
	 ...
	 * label b-tree during its operations.
	 * 
	 ...
	 * 
	 * @return Return a LabelBTreeHeaderPage object that is the header page
	 *         of this btree file.
	 */
	 ...
	 * 
	 * @param filename the Label B+ tree file name. Input parameter.
	 * @exception GetFileEntryException  can not ger the file from DB
	 * @exception PinPageException       failed when pin a page
	 * @exception ConstructPageException Label BT page constructor failed
	 */
	public LabelBTreeFile(String filename)
		...
		/*
		 *
		 * - labelheaderPageId is the PageId of this LabelBTreeFile's header page;
	...
	/**
	 * Close the label B+ tree file. Unpin header page.
	 * 
	...
	/**
	 * Destroy entire Label B+ tree file.
	 * 
	 * @exception IOException              error from the lower layer
	 ...
	 * @exception ConstructPageException   error in Label BT page constructor
	 * @exception PinPageException         failed when pin a page
	 */
	...
	private void _destroyFile(PageId pageno)
		...
		if (sortedPage.getType() == NodeType.INDEX) {
			...
			PageId childId;
			KeyDataEntry entry;
			for (entry = indexPage.getFirst(lid); entry != null; entry = indexPage.getNext(lid)) {
	...
	/**
	 * insert label with the given key and lid
	 * 
	 * @param key the key of the label. Input parameter.
	 * @param lid the lid of the label. Input parameter.
...

	{
		...
		if (headerPage.get_rootId().pid == INVALID_PAGE) {
			...
			newRootPage.insertLabel(key, lid);
...

	{
		...
		// - pageType == LEAF:
		// try to insert pair (key, lid), maybe split
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			if (currentIndexPage.available_space() >= LabelBT.getKeyDataLength(upEntry.key, NodeType.INDEX)) {
			...
			for (tmpEntry = currentIndexPage.getFirst(delLid); tmpEntry != null; tmpEntry = currentIndexPage
					.getFirst(delLid)) {
				newIndexPage.insertKey(tmpEntry.key,
						((IndexData) tmpEntry.data).getData());
				currentIndexPage.deleteSortedLabel(delLid);
			}

			// ASSERTIONS:
			// - currentIndexPage empty
			// - newIndexPage holds all former labels from currentIndexPage

			// we will try to make an equal split
			LID firstLid = new LID();
			KeyDataEntry undoEntry = null;
			for (tmpEntry = newIndexPage.getFirst(firstLid); (currentIndexPage.available_space() > newIndexPage
					.available_space()); tmpEntry = newIndexPage.getFirst(firstLid)) {
				// now insert the <key,pageId> pair on the new
				// index page
				undoEntry = tmpEntry;
				currentIndexPage.insertKey(tmpEntry.key,
						((IndexData) tmpEntry.data).getData());
				newIndexPage.deleteSortedLabel(firstLid);
			}

			// undo the final label
			if (currentIndexPage.available_space() < newIndexPage.available_space()) {
				...
				currentIndexPage.deleteSortedLabel(new LID(currentIndexPage.getCurPage(),
			...
			if (LabelBT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
				...
				tmpEntry = LabelBT.getEntryFromBytes(currentIndexPage.getpage(),
				...
				currentIndexPage.deleteSortedLabel(new LID(currentIndexPage.getCurPage(), i));
			...
			// delete first label on new index page since it is given up
			newIndexPage.deleteSortedLabel(delLid);
			...
			LabelBTLeafPage currentLeafPage = new LabelBTLeafPage(page, headerPage.get_keyType());
			...
			// check whether there can still be entries inserted on that page
			if (currentLeafPage.available_space() >= LabelBT.getKeyDataLength(key, NodeType.LEAF)) {
				// no split has occurred

				currentLeafPage.insertLabel(key, lid);
			...
			for (tmpEntry = currentLeafPage.getFirst(firstLid); tmpEntry != null; tmpEntry = currentLeafPage
					.getFirst(firstLid)) {

				newLeafPage.insertLabel(tmpEntry.key,
						((LabelLeafData) (tmpEntry.data)).getData());
				currentLeafPage.deleteSortedLabel(firstLid);

			}

			// ASSERTIONS:
			// - currentLeafPage empty
			// - newLeafPage holds all former labels from currentLeafPage

			KeyDataEntry undoEntry = null;
			for (tmpEntry = newLeafPage.getFirst(firstLid); newLeafPage.available_space() < currentLeafPage
					.available_space(); tmpEntry = newLeafPage.getFirst(firstLid)) {
				undoEntry = tmpEntry;
				currentLeafPage.insertLabel(tmpEntry.key,
						((LabelLeafData) tmpEntry.data).getData());
				newLeafPage.deleteSortedLabel(firstLid);
			}

			if (LabelBT.keyCompare(key, undoEntry.key) < 0) {
				// undo the final label
				if (currentLeafPage.available_space() < newLeafPage.available_space()) {
					newLeafPage.insertLabel(undoEntry.key,
							((LabelLeafData) undoEntry.data).getData());

					currentLeafPage.deleteSortedLabel(new LID(currentLeafPage.getCurPage(),
							(int) currentLeafPage.getSlotCnt() - 1));
				}
			}

			// check whether <key, lid>
			// will be inserted
			// on the newly allocated or on the old leaf page

			if (LabelBT.keyCompare(key, undoEntry.key) >= 0) {
				// the new data entry belongs on the new Leaf page
				newLeafPage.insertLabel(key, lid);
	 ...
	 * `lid' is IN the data entry; it is not the id of the data entry)
	 * 
	 * @param key the key in pair <key, lid>. Input Parameter.
	 * @param lid the lid in pair <key, lid>. Input Parameter.
	 * @return true if deleted. false if no such label.
	 ...
	 * @exception FreePageException         error in Label BT page constructor
	 * @exception RecordNotFoundException   error delete a label in a Label BT page
	 * @exception PinPageException          error when pin a page
	 ...
	 * lo_key is null.
	 * 
	 * Starting label returned in *pstartlid, on page *pppage, which is pinned.
	 *
	 ...
	 * 
	 * @param startlid it will reurn the first lid =< lo_key
	 * 
	...
	LabelBTLeafPage findRunStart(KeyClass lo_key,
		...
		while (curEntry == null) {
			...
			if (nextpageno.pid == INVALID_PAGE) {
				// oops, no more labels, so set this scan to indicate this.
				return null;
			}
		...
		// ASSERTIONS:
		// - curkey, curLid: contain the first label on the
	...
	private boolean NaiveDelete(KeyClass key, LID lid)
		...
		leafPage = findRunStart(key, curLid); // find first page,lid of key
		if (leafPage == null)
			return false;
	 ...
	 * LabelBTLeafPage::delUserlid.
	 * 
	 * @return false if no such label; true if succees
	 */
	...
	private KeyClass _Delete(KeyClass key,
				...
				if (leafPage.delEntry(new KeyDataEntry(key, lid))) {
					...
					if ((4 + leafPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
						...
						if (siblingPage.redistribute(leafPage, parentPage,
							...
							for (tmpEntry = rightChild.getFirst(firstLid); tmpEntry != null; tmpEntry = rightChild
									.getFirst(firstLid)) {
								leftChild.insertLabel(tmpEntry);
								rightChild.deleteSortedLabel(firstLid);
							}
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			if ((4 /* sizeof slot */ + indexPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
				...
				int pushKeySize = 0;
				if (direction == 1) {
					pushKeySize = LabelBT.getKeyLength(parentPage.findKey(siblingPage.getFirst(new LID()).key));
				} else if (direction == -1) {
					pushKeySize = LabelBT.getKeyLength(parentPage.findKey(indexPage.getFirst(new LID()).key));
				}
				...
				if (siblingPage.redistribute(indexPage, parentPage,
					...
					// move all entries from rightChild to leftChild
					for (KeyDataEntry tmpEntry = rightChild.getFirst(firstLid); tmpEntry != null; tmpEntry = rightChild
							.getFirst(firstLid)) {
	...
	void trace_children(PageId id)
		...
		if (trace != null) {
			...
			if (sortedPage.getType() == NodeType.INDEX) {
				...
				for (entry = indexPage.getFirst(metaLid); entry != null; entry = indexPage.getNext(metaLid)) {
				...
				for (entry = leafPage.getFirst(metaLid); entry != null; entry = leafPage.getNext(metaLid)) {
