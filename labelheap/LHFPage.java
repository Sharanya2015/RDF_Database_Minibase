...
interface LConstSlot {
    int INVALID_SLOT = -1;
    int EMPTY_SLOT = -1;
}

public class LHFPage extends Page
        implements LConstSlot, GlobalConst {
     ...
     * open a LHFPage and make this LHFpage piont to the given page
     * 
    ...
    public LID insertLabel(byte[] label)
        ...
        int spaceNeeded = labelLen + SIZE_OF_SLOT;
        ...
        if (spaceNeeded > freeSpace) {
            ...
            usedPtr = Convert.getShortValue(USED_PTR, data);
            usedPtr -= labelLen; // adjust usedPtr
            Convert.setShortValue(usedPtr, USED_PTR, data);

            // insert the slot info onto the data page
            setSlot(i, labelLen, usedPtr);

            // insert data onto the data page
            System.arraycopy(label, 0, data, usedPtr, labelLen);
            curPage.pid = Convert.getIntValue(CUR_PAGE, data);
     ...
     * @exception IOException                     I/O errors
     *                                            in C++ Status deletelabel(const
     *                                            LID& lid)
     */
    public void deleteLabel(LID lid)
            throws IOException,
            LabelInvalidSlotNumberException {
        int slotNo = lid.slotNo;
        short labelLen = getSlotLength(slotNo);
        slotCnt = Convert.getShortValue(SLOT_CNT, data);

        // first check if the label being deleted is actually valid
        if ((slotNo >= 0) && (slotNo < slotCnt) && (labelLen > 0)) {
            ...
            throw new LabelInvalidSlotNumberException(null, "LABELHEAPFILE: INVALID_SLOTNO");
        }
    }
     ...
     * <br>
     * Status getlabel(LID lid, char *labelPtr, int& labelLen)
     * 
     * @param lid the label ID
    ...
    public Label getLabel(LID lid)
        ...
        labelLen = getSlotLength(slotNo);
        slotCnt = Convert.getShortValue(SLOT_CNT, data);
        if ((slotNo >= 0) && (slotNo < slotCnt) && (labelLen > 0)
                && (pageNo.pid == curPage.pid)) {
            offset = getSlotOffset(slotNo);
            label = new byte[labelLen];
            System.arraycopy(data, offset, label, 0, labelLen);
            Label nlabel = new Label(label, 0, labelLen);
            return nlabel;
        }
    ...
    public Label returnLabel(LID lid)
        ...
        if ((slotNo >= 0) && (slotNo < slotCnt) && (labelLen > 0)
            ...
            offset = getSlotOffset(slotNo);
            Label label = new Label(data, offset, labelLen);
            return label;
        }
